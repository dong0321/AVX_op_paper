...
\begin{abstract}
  ...
  these optimizations, we achieve a higher efficiency for local
...
\begin{enumerate}
  ...
  speed up a variety type \mpi reduction operations.\todo{I don't understand}
%
...
Intel Advanced Vector Extension 2 (Intel AVX2), is a significant improvement to Intel ISA.
...
using C/C++ style coding instead of assembly language. Without support for such intrinsic,
users would have to write assembly code directly to manipulate SIMD
instructions.
...
We implement our advanced reduction operation with AVX, AVX2, AVX-512 support
...
automatically detect the processor information and check related flags, select supported AVX reduction operations or fallback to the default basic module if it is not supported by the
hardware, as shown in Figure~\ref{fig:512flow}. 
%
To be more specific and precise, we explicitly check CPUID -- a processor
...
(a) relying on automatic vectorization support that provided by the compiler;
...
guidelines and restrictions for vectorizable code, which largely depends on the capability and efficiency of a specific compiler.
%
And programmers also need to be aware of the specifics of the instructions that are supported by a processor.
...
%
For the second method, it allows excellent control over the very low-level instruction stream, but the use of
intrinsics in every application is time-consuming and error-prone for application programmers and users.
%
For our work, to facilitate and promote the use of AVX-512 features,
we prefer to adapt those advantages of long vectors load/store and computation 
into a widely-used programming model -- \ompi, we use intrinsics and compiling flags together and concurrently
to guide the compiler in the vectorization phase to achieve outstanding performance.
...
floating-point, also supports logical reduction operations for an integer type.
...
Eventually, we reach the last section of the optimization.
Significant vectorized loop execution time is often spent in remainder
loops dealing with just a few elements that cannot full-fill a vector.
...
%
For this part of the remainder, we use Duff's device which we manually implement
...
We can see our implementation supports most combination of types and operations in \mpi standard.
"-" indicates the logistic operations that are not applicable for floating-point.
Table\ref{tab:parameters1} lists the supported x86 instruction set architectures and related CPU flags from legacy SSE to the latest AVX-512 instruction sets.
%
To be noted, our work mainly based on the "Fundamental" feature instruction set with flag AVX512F, available on Knights Landing processors and Intel Xeon processors. It contains vectorized arithmetic operations, comparisons, type conversions,
data movement, data permutation, bitwise logical operations on vectors and masks, and miscellaneous
math functions. This is similar to the core feature sets of the AVX2 instructions, with
...
\begin{table}
  \centering
  \caption{Supported ISA and CPU flags}\label{fig:cpuflags}
...
In this section, we study the performance benefits of our AVX-512 optimized \ompi reduction
operation by Performance API (PAPI)~\cite{papi} -- a tool that can measure
application performance in these increasingly complex environments 
...
(2) Branching instructions: number of branch execution instructions including branch instructions taken and not-taken, instructions mispredicted and instructions correctly predicted, which have a significant impact on performance.
...
\begin{figure}[h]
    ...
    \caption{Comparison between AVX-512 optimized OMPI and default OMPI for MPI\_SUM reduction with PAPI (summary of different types of instruction)}
    \label{fig:papi_ins}
\end{figure}
...
Figure~\ref{fig:papi_ins} lists the total number of all instructions, and memory access instructions of
...
element-wise reduction method. By using long vectors, we largely decreased the "for loop" of the reduction operation.
Consequently, the AVX-512 code has much less control and branching instructions.
Which means we have less conditional branch instructions.
Especially, for conditional branch instructions not taken, we gain
more benefits compare to others, which shows more conditional branch instructions are being correctly predicted.
...
it processes two elements. Our optimized implementation use AVX-512 vector instruction
...
Results demonstrate that with AVX-512 optimized operation, it is 10X faster than element-wise operation.
To be more specific, when the total size of the reduction elements is small, the performance benefit is
not significant noticeable. However, when the buffer size bigger than 4KB, the performance advantage becomes consistent and stable.
We also compare MPI operation together with memcpy, which indicates the peak memory bandwidth.
To make a fair comparison, we list the complete execution sequence of reduction operation and memcpy.
...
The result shows that even with an additional computation included, and our optimized AVX-512 reduction operation achieves
...
\begin{figure}[h]
    ...
    \caption{Comparison of MPI\_SUM with AVX-512 optimized OMPI and default OMPI together with memcpy}
    \label{fig:avx_sum}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Comparison of MPI\_BAND with AVX-512 optimized OMPI and default OMPI together with memcpy}
    \label{fig:avx_band}
\end{figure}
...
In this paper, we demonstrated the benefits of Intel AVX, AVX2 and AVX-512 vector operations. We
addressed the performance advantages of different features introduced by AVX
with longer vector length. Furthermore, we extended our investigation and analysis to introduce
...
\mpifunc{Local_reduce}. Experiments are conducted on an Intel Xeon Gold cluster,
which shows with AVX-512 optimized reduction operations, we achieve 10X performance benefits.
%
...
application using distributed model Horovod, which uses \ompi allreduce to calculate and update the gradient to adjust the weights.
Just by switching to our new reduction strategy, we achieve a speedup of 12.38\% with 1536 processes.
This provides a better solution for reduction operation
in distributed neural network training applications whose performance is limited by the time-consuming reduce calculation, because the number of reductions and the data amount are huge.
%
