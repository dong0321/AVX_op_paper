...
\begin{figure}[h]
    \centering
    % trim={<left> <lower> <right> <upper>}
    \includegraphics[width=\linewidth]{avx_extend_more_sum_u8_1k-128M.pdf}\vspace{-0.5cm}
...
\begin{figure}[h]
    \centering
    % trim={<left> <lower> <right> <upper>}
    \includegraphics[width=\linewidth]{avx_extend_more_prod_u8_1k-128M.pdf}\vspace{-0.5cm}
...
\section{Experimental evaluation with other platforms}\label{sec:experiments}
\subsection{AMD Zen 2 architecture}
AMD's new Zen architecture supports all the latest x86 instructions such as SSE and AVX1/2.
...
On the Zen 2 architecture, it doubles the width of the physical registers, execution units, and data paths to 256 bits.
...
\begin{figure}[h]
    \centering
    % trim={<left> <lower> <right> <upper>}
    \includegraphics[ width=\linewidth]{amd256_extend_more_sum_u8_1k-128M.pdf}\vspace{-0.5cm}
    ...
    \label{fig:amd_sum}%\vspace{-0.3cm}
\end{figure}

We conducted our benchmark experiments on a EPYC 7302 processor based cluster,
which is based on the Zen 2 microarchitecture with a base frequency of 3.0 GHz.
It supports AVX1 and AVX2 instructions.

Figure~\ref{fig:amd_sum} show the result for the
...
C language extension using intrinsic. As we mentioned in this paper, \ompi is using modular architecture,
...
Figure~\ref{fig:arm_sum} shows the results of addition operation of \ompi default implementation,
...
\begin{algorithm}[t]
\caption{Arm SVE based reduction algorithm}\label{fig:reduce_algorithm}
...
%\If {( $blocklen$ $\geqslant$ $svcntb$ )}
  ...
  \State $types\_per\_step$ = svcntb | svcnth | svcntw | svcntd
  % \EndIf
  \State{\#pragma unroll}
\For { $k \gets types\_per\_step $ to $ count$}
...
\begin{figure}[h]
    \centering
    % trim={<left> <lower> <right> <upper>}
    \includegraphics[width=\linewidth]{sve512_extend_more_sum_u8_1k-128M.pdf}\vspace{-0.5cm}
    ...
    \label{fig:arm_sum}\vspace{-0.5cm}
\end{figure}
